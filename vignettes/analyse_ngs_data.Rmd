---
title: "Analyse NGS data using CloneDetective"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{analyse_ngs_data}
...
This vignette focuses on using the package to analyse NGS data.
NGS data refers to lineage tracing DNA barcoding data which exclusively sequences 
clone barcode reads using Next Generation Sequencing (hence called NGS data).

We will go through how to analyse clone barcode distributions from NGS data which 
...
Before we delve into investigating what our NGS data looks like, we have to load it in first.
...
from the 8k and 10k NGS data we used in our paper.
...
clones_nxclone <- fread(
  system.file("extdata", "ngs_clone_barcode_counts_subsampled.csv", 
...
1, 20, 200, 1000 cells.
Remember, we are using sub-sampled NGS data, that is why the `at_least_1_cells` column
for both samples have exactly 4,000 clones.

This information is useful to see how many reasonably sized clones we get in our NGS data.
...
# Using NGS data to plan future scRNAseq experiment
